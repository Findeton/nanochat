...
TAG="d12-memory-phase36-branch-recovery-from35-250"
...
echo "Base checkpoint: d12-memory-phase35-branch-from3-750 step 250"
echo "Log: ${LOG}"

python3 -u scripts/chat_memory.py \
  --run dummy \
  --device-type cuda \
  --model-tag d12-memory-phase35-branch-from3-750 \
  --model-step 250 \
  --output-tag "${TAG}" \
  --num-iterations 500 \
  --phase36-steps 500 \
  ...
  --phase36-branch-recovery-loss-weight 1.0 \
  --phase36-branch-recovery-batch-frac 0.75 \
  --phase36-branch-recovery-steps 16 \
  --phase36-branch-recovery-temperature 0.35 \
  --phase36-branch-recovery-top-k 12 \
  --phase36-branch-min-prefix-tokens 3 \
  --phase36-branch-recovery-max-target-tokens 12 \
  --phase36-branch-recovery-post-key-tokens 8 \
  --phase36-branch-margin-loss-weight 0.75 \
